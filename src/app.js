...
const routes = [
  ['/', 'Home', '⌂'], ['/lecture', 'Lecture', '◉'], ['/notes', 'Notes', '▤'],
  ['/schedule', 'Schedule', '□'], ['/board', 'Board', '◌'], ['/chat', 'AI Chatbot', '✦'], ['/profile', 'My Page', '☺']
];
...
const icons = { lecture: '◉', notes: '▤', schedule: '□', board: '◌', chat: '✦' };
...
function nav() {
  ...
  return `<aside class="sidebar"><a class="brand" data-route="/" href="/"><span>✦</span> Study Mate</a><p class="side-label">${appLanguage === 'ko' ? '메뉴' : 'MENU'}</p><nav>${routes.map(([url, label, icon]) => `<a class="nav-link ${path === url ? 'active' : ''}" data-route="${url}" href="${url}"><i>${icon}</i>${t(label)}</a>`).join('')}</nav><div class="side-help"><span>?</span><div><strong>${appLanguage === 'ko' ? '도움이 필요하신가요?' : 'Need help?'}</strong><small>${appLanguage === 'ko' ? 'Study Mate에게 물어보기' : 'Ask Study Mate'}</small></div></div></aside>`;
}
function topbar(title) { const displayTitle = /^Good morning,/.test(title) && appLanguage === 'ko' ? `안녕하세요, ${student.name}님!` : t(title); return `<header class="topbar"><button class="menu-button" data-menu aria-label="Open menu">☰</button><div><p class="eyebrow">STUDY MATE</p><h1>${displayTitle}</h1></div><div class="profile-chip"><select id="app-language" aria-label="App language"><option value="en" ${appLanguage === 'en' ? 'selected' : ''}>EN</option><option value="ko" ${appLanguage === 'ko' ? 'selected' : ''}>한국어</option></select><span class="avatar">${initials}</span><span class="profile-name">${student.name}</span></div></header>`; }
...
function empty(title, copy) { return `<div class="state empty"><span>◌</span><strong>${title}</strong><p>${copy}</p></div>`; }
...
function showSttNotice(message, isError = false) { const notice = document.querySelector('#stt-notice'); if (notice) notice.innerHTML = `${isError ? '!' : '⌁'} <span><strong>STT status:</strong> ${message}</span>`; }
...
function openKeywordPopup(keyword, noteOverride = null) { const selected = noteOverride || noteState.notes[noteState.selectedLectureId]; const detail = selected?.keywordDetails?.[keyword]; if (!detail) return; document.querySelector('#keyword-popup')?.remove(); document.body.insertAdjacentHTML('beforeend', localizeUi(`<div id="keyword-popup" class="keyword-modal" role="dialog" aria-modal="true"><div class="keyword-dialog"><button class="modal-close" data-note-action="close-keyword" aria-label="Close">×</button><p class="tag">KEYWORD GUIDE</p><h2>${esc(keyword)}</h2><dl><div><dt>Basic meaning</dt><dd>${esc(detail.meaning)}</dd></div><div><dt>In this lecture</dt><dd>${esc(detail.context)}</dd></div><div><dt>For your major</dt><dd>${esc(detail.major)}</dd></div><div><dt>Study tip</dt><dd>${esc(detail.studyTip)}</dd></div></dl></div></div>`)); }
...
function renderBoard() {
  ...
  if (selected) {
    ...
    return `${topbar('Board')}<main><button class="back-link" data-board-action="back">← ${ui('All posts', '전체 게시글')}</button>${postContent}</main>`;
  }
  const list = posts.map(post => `<button class="board-post" data-board-action="open" data-post-id="${post.id}"><span class="post-icon">◌</span><div><span class="pill ${post.status === 'Answered' ? 'success' : 'warm'}">${statusLabel(post.status)}</span><h3>${esc(post.title)}</h3><p>${categoryLabel(post.category)} · ${esc(post.author)} · ${(post.comments || []).length} ${ui('replies', '댓글')}</p></div><b>›</b></button>`).join('') || empty(ui('No posts found', '게시글이 없습니다'), ui('Try another keyword or category.', '다른 검색어 또는 카테고리를 시도해 보세요.'));
...
function renderChat() {
  const messages = chatState.messages.map(message => `<div class="chat-message ${message.role}"><span>${message.role === 'assistant' ? '✦' : initials}</span><div><p>${esc(message.text)}</p>${message.opportunities ? opportunityCards(message.opportunities) : ''}</div></div>`).join('');
  ...
  return `${topbar('AI Chatbot')}<main class="chat-page"><div class="page-intro center"><p class="tag">STUDY MATE AI · PHASE 5</p><h2>${appLanguage === 'ko' ? '나만의 학습 도우미' : 'Your study companion.'}</h2><p>${appLanguage === 'ko' ? '학습 질문과 청년 지원 정책을 물어보세요.' : 'Ask about study support and youth policies.'}</p></div><section class="chat-window"><div class="chat-messages">${messages}${chatState.loading ? `<div class="chat-message assistant"><span>✦</span><div>${esc(chatState.loadingLabel || 'Thinking…')}</div></div>` : ''}</div><div class="suggestions"><button data-chat-action="prompt" data-prompt="How do I record a lecture?">${appLanguage === 'ko' ? '강의 녹음 방법' : 'How do I record a lecture?'}</button><button data-chat-action="prompt" data-prompt="What assignments are due this week?">${appLanguage === 'ko' ? '이번 주 과제' : 'Upcoming assignments'}</button><button data-chat-action="prompt" data-prompt="청년 정책을 추천해줘">${appLanguage === 'ko' ? '청년 정책 추천' : 'Recommend youth policies'}</button></div><div class="chat-input"><input id="chat-message" placeholder="${appLanguage === 'ko' ? 'Study Mate에게 질문하세요…' : 'Ask Study Mate anything…'}" aria-label="Chat message"/><button data-chat-action="send" ${chatState.loading ? 'disabled' : ''}>↑</button></div></section><p class="chat-note">${integrationNote}</p></main>`;
}
...
function render() { persistAppState(); const path = location.pathname; const reviewMatch = path.match(/^\/lectures\/([^/]+)$/); const pages = {'/': home, '/lecture': lecture, '/notes': notes, '/schedule': schedule, '/board': board, '/chat': chat, '/profile': profile}; const page = reviewMatch ? lectureReview : (pages[path] || home); if (!reviewMatch && !pages[path]) history.replaceState({}, '', '/'); if (!reviewMatch && reviewState.audioUrl) releaseReviewAudio(); app.innerHTML = localizeUi(`${nav()}<div class="overlay" data-close-menu></div><div class="shell">${page()}</div><div class="mobile-nav">${routes.slice(0, 5).map(([url, label, icon]) => `<a data-route="${url}" href="${url}" class="${location.pathname === url ? 'active' : ''}"><i>${icon}</i><small>${t(label)}</small></a>`).join('')}</div>`); if (path === '/lecture') mountSttDebug(); if (reviewMatch) mountLectureReview(recordings.find(item => item.id === decodeURIComponent(reviewMatch[1]))); }
function home() {
